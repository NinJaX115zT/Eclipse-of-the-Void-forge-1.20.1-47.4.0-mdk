...
public class EtherealBeeGrowCropGoal extends Goal {
    ...
    public void tick() {
        ...
        double distanceToCrop = bee.distanceToSqr(targetCropPos.getX() + 0.5D, targetCropPos.getY() + 0.5D, targetCropPos.getZ() + 0.5D);
        
        // Visual cue: if ready to grow, subtle particles
        if (bee.level() instanceof ServerLevel serverLevel && bee.tickCount % 10 == 0 && distanceToCrop < 4.0D) {
            serverLevel.sendParticles(net.minecraft.core.particles.ParticleTypes.HAPPY_VILLAGER,
                    bee.getX(), bee.getY() + 0.5D, bee.getZ(),
                    2, 0.1, 0.1, 0.1, 0.0);
        }

        if (distanceToCrop < 4.0D) {
            ...
            if (state.getBlock() instanceof BonemealableBlock crop && crop.isValidBonemealTarget(bee.level(), targetCropPos, state, false)) {
                ...
                // Stop navigation and reset movement
